...
Last name: MONLOY
Language: Javascript
Paradigm(s): Object-Oriented (Template Method Pattern, Factory Method Pattern))
********************/
...
async function generateReports() {
    ...
    // Define reports to generate, makes it easy to add more types in the future
    const reportsToGenerate = ['REPORT_1', 'REPORT_2', 'REPORT_3'];

    // Loop through report types, use Factory to create each report
    for (const reportType of reportsToGenerate) {
        try {
            const report = ReportFactory.createReport(reportType, cleanedFilteredRows);
            await report.generate();
        } catch (err) {
            console.error(`Error generating ${reportType}:`, err);
        }
    }
